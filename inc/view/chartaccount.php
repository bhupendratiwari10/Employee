...
<div class='box' style='margin-bottom: 28px;padding-bottom: 28px;border-bottom: 1px dashed;'>
    ...
    // Process payments made (paid_through)
    ...
    while($nala = mysqli_fetch_assoc($kl)){ 
        ...
        $date = $nala['payment_date']; 
        $ptype = $nala['payment_type'];
        ...
        // Define $ac based on available data
        if(!empty($acy)){
            $ac = "Vendor: ".$acy;
        } else if(!empty($acx)){
            $ac = "Account: ".$acx;
    ...
    // Process payments made (deposit_to)
    ...
    while($nala = mysqli_fetch_assoc($kl)){ 
        ...
        // Define $ac for deposit_to transactions
        $acy = namebyAid($nala['vendor_id'], "company_name", "zw_company");
        $acx = namebyAid($nala['paid_through'], "account_name", "zw_accounts");
    ...
    // Account Payable (id=83)
    if($id==83){ 
        $kil = mysqli_query($con,"SELECT * FROM `zw_Bill` WHERE status='1'");
        while($naila = mysqli_fetch_assoc($kil)){ 
            $pyid = $naila['id']; 
            $inrvall = $naila['subTotal'];
            $date = $naila['bill_date'];        
            $aic = "#ZW-BILL-".$pyid;
    ...
    // Account Receivable (id=84)
    if($id==84){ 
        $kil = mysqli_query($con,"SELECT * FROM `zw_invoices` WHERE status='1'");
        while($naila = mysqli_fetch_assoc($kil)){ 
            $pyid = $naila['id']; 
            $inrvall = $naila['subTotal'];
            $date = $naila['invoice_date'];        
            $aic = "#ZW-INV-".$pyid;
            
            echo"<div class='col-12 p-0 m-0 row' style='color:#333;'>";
                echo"<div class='col-md-2'>".str_replace("00:00:00", "", $date)."</div>";
                echo"<div class='col-md-3'>$aic </div>";
                echo"<div class='col-md'>Invoice</div>";
                echo"<div class='col-md'>₹ $inrvall</div>";
                echo"<div class='col-md'></div>";
            echo"</div><hr>";
            $tt = $tt+$inrvall;
        ...
        $kil = mysqli_query($con,"SELECT * FROM `zw_epr_invoices` WHERE status='1'");
        while($naila = mysqli_fetch_assoc($kil)){ 
            $pyid = $naila['id']; 
            $inrvall = $naila['subTotal'];
            $date = $naila['invoice_date'];        
            ...
            echo"<div class='col-12 p-0 m-0 row' style='color:#333;'>";
                echo"<div class='col-md-2'>".str_replace("00:00:00", "", $date)."</div>";
                echo"<div class='col-md-3'>$aic </div>";
                echo"<div class='col-md'>EPR Invoice</div>";
                echo"<div class='col-md'>₹ $inrvall</div>";
                echo"<div class='col-md'></div>";
            echo"</div><hr>";
            $tt = $tt+$inrvall;
    ...
    while($nala = mysqli_fetch_assoc($kils)){ 
        ...
        $debit = $nala['debit']; 
        $credit = $nala['credit'];
        $date = substr($nala['time_str'],0,11);
        $customer = namebyAid($nala['contact'], "customer_display_name", "zw_customers");
        
        if(!empty($customer)){
            $aic = "Contact: $customer";
        } else {
            $aic = "Journal Entry #$pyid";
        }
    ...
    if($balance!=$tt){
        mysqli_query($con,"UPDATE zw_accounts SET closing_balance='$tt' where id = $id");
    }
