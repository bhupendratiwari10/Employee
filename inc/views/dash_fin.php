...
?>
                ...
                <div style='' class='col-md-6 m-0 p-0 py-2 row'>
                    ...
                    <div class='col-6 m-0 p-0' style='transform:scale(0.95);box-shadow: 1px 11px 11px -6px #dbdbdb;border-radius: 12px;border: 1px solid #b2b2b24c;overflow:hidden;'>
                        ...
                        <div style="display:flex;flex-direction: column;align-items: center;width: 100%;">
                            ...
                            <p style='margin:0px 0px;color:gray;'><small>CTO - ZWGLOBAL</small></p><br>
                        </div>
            ...
            $totalBalanceQuery = generalQuery("SELECT SUM(closing_balance) as closing_balance  FROM `zw_accounts` WHERE account_type='14'");
            if(!empty($totalBalanceQuery)){while($row = mysqli_fetch_assoc($totalBalanceQuery)){$closingBalanceIncome = $row['closing_balance'];}}   ?>
                ...
                <div class='col-md-12 p-0 m-0 row' style=''>
                    ...
                    <div class='px-3 pt-4 pb-0 col-md-6' style='transform:scale(0.95);box-shadow: 1px 11px 11px -6px #dbdbdb;border-radius: 12px;border: 1px solid #b2b2b24c;overflow:hidden;display: flex;flex-direction: column;justify-content: space-around;aspect-ratio: 1 / 0.63;'>
                        ...
                        <?php $expqi = mysqli_query(dbCon(),"SELECT sum(subTotal) as amount FROM zw_Bill"); $rxxw = mysqli_fetch_assoc($expqi);$blbal = $rxxw['amount'];  ?>
                        <h3 style='font-weight:800;'><?php echo $extbal+$blbal;?> ₹</h3></div>
                        <div class='col-12 m-0 p-0 row'>
                    ...
                    <div class='px-3 pt-4 m-0 pb-0 col-md-6' style='transform:scale(0.95);box-shadow: 1px 11px 11px -6px #dbdbdb;border-radius: 12px;border: 1px solid #b2b2b24c;overflow:hidden;display: flex;flex-direction: column;justify-content: space-around;'>
                        ...
                        <h3 style='font-weight:800;'><?php echo $closingBalanceIncome-($extbal+$blbal); ?> ₹</h3></div>
                        <div class='col-12 m-0 p-0 row'>
                            <div class='col-5' style="display: flex;flex-direction: column;justify-content: flex-end;"><b style='color:#57be87;'><i class='mi-arrow-up-circle-fill'></i> 15%</b></div><div class='col'></div>
            ...
            <div class='col-md-3 m-0 p-0 my-2  row' style=''>
                <div class='px-3 pt-4 pb-0 mb-3 col-md-12' style='box-shadow: 1px 11px 11px -6px #dbdbdb;border-radius: 12px;border: 1px solid #b2b2b24c;overflow:hidden;display: flex;flex-direction: column;justify-content: space-around;'>
                    ...
                    <h3 style='font-weight:800;'><?php echo countRows('zw_customers WHERE status=1'); ?> +</h3></div>
                    <div class='col-12 m-0 p-0 row'>
                            <div class='col-5' style="display: flex;flex-direction: column;justify-content: flex-end;"><b style='color:#57be87;'><i class='mi-arrow-up-circle-fill'></i> 15%</b></div><div class='col'></div>
...
$qcf = "SELECT SUM(CASE WHEN payment_type = 'received' AND deposit_to IN (SELECT id FROM zw_accounts WHERE account_type = '3' OR account_type = '4') THEN payment_made ELSE 0 END) AS payments_received,
...
FROM `zw_payment_made`
  ...
  LEFT JOIN `zw_journal_items` ON MONTH(payment_date) = MONTH(payment_date)
...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  $seriesData[$month] = [
    ...
    'journal_debit' => $row['journal_debit'],
    'journal_credit' => $row['journal_credit'],
    //'journal_net' => $row['journal_debit'] - $row['journal_credit'],  // Calculate journal net here
    'cashflow' => $row['payments_received'] - ($row['payments_made'] + $row['journal_credit']) + $row['expenses'] - $row['journal_debit'],
  ];
      //print_r($seriesData[$month])."<br><br>";
}
...
// Combine filtering conditions into subqueries for cleaner structure
$payments_received_query = "SELECT SUM(payment_made) AS payments_received 
                             FROM zw_payment_made 
                             WHERE payment_type='received' 
                             AND deposit_to IN (SELECT id FROM zw_accounts WHERE account_type IN (3, 4))";

$payment_made_query = "SELECT SUM(payment_made) AS payment_made 
                        FROM zw_payment_made 
                        WHERE payment_type='made' 
                        AND paid_through IN (SELECT id FROM zw_accounts WHERE account_type IN (3, 4))";

$expenses_query = "SELECT SUM(amount) AS amount 
                   FROM zw_expense 
                   WHERE paid_throw IN (SELECT id FROM zw_accounts WHERE account_type IN (3, 4))";

$journal_credit_query = "SELECT SUM(credit) AS credit 
                         FROM zw_journal_items 
                         WHERE account_id IN (SELECT id FROM zw_accounts WHERE account_type IN (3, 4))";

$journal_debit_query = "SELECT SUM(debit) AS debit 
                        FROM zw_journal_items 
                        WHERE account_id IN (SELECT id FROM zw_accounts WHERE account_type IN (3, 4))";

// Execute queries and handle potential errors (replace with actual error handling)
$payments_received_result = mysqli_query($con, $payments_received_query) or die(mysqli_error($con));
$payment_made_result = mysqli_query($con, $payment_made_query) or die(mysqli_error($con));
$expenses_result = mysqli_query($con, $expenses_query) or die(mysqli_error($con));
$journal_credit_result = mysqli_query($con, $journal_credit_query) or die(mysqli_error($con));
$journal_debit_result = mysqli_query($con, $journal_debit_query) or die(mysqli_error($con));

// Fetch data and calculate incoming/outgoing
$payments_received = mysqli_fetch_assoc($payments_received_result)['payments_received'];
$payment_made = mysqli_fetch_assoc($payment_made_result)['payment_made'];
$exp_amount = mysqli_fetch_assoc($expenses_result)['amount'];
$td_credit = mysqli_fetch_assoc($journal_credit_result)['credit'];
$td_debit = mysqli_fetch_assoc($journal_debit_result)['debit'];

$incoming = $payments_received + $td_debit;
$outgoing = $payment_made + $exp_amount + $td_credit;

// Free results (replace with actual mysqli_free_result)
mysqli_free_result($payments_received_result);
mysqli_free_result($payment_made_result);
mysqli_free_result($expenses_result);
mysqli_free_result($journal_credit_result);
mysqli_free_result($journal_debit_result);



$ct = mysqli_query($con,"SELECT SUM(closing_balance) AS balance FROM zw_accounts WHERE account_type=3");$ctx=mysqli_fetch_assoc($ct); $cashtodayx = $ctx['balance'];
$cashtoday = $incoming-$outgoing;
...
?>
            ...
            <div class='row col-12 m-0 p-0'>
                ...
                <div class='col'><small style='font-size:76%;font-weight:600;color:green;'>Incoming</small><br><b style='font-size:111%;'> ₹ <?php echo $incoming; ?></b></div>
                <div class='col'><small style='font-size:76%;font-weight:600;color:orange;'>Outgoing</small><br><b style='font-size:111%;'> ₹ <?php echo $outgoing; ?></b></div>
                <div class='col'><small style='font-size:76%;font-weight:600;color:gray;'>Cash as of today</small><br><b style='font-size:111%;'> ₹ <?php echo $cashtoday; ?></b></div>
            </div>
            </div>
