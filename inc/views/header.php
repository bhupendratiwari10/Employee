...
// Check if user_role cookie exists and is valid
if (isset($_COOKIE['user_role']) && is_numeric($_COOKIE['user_role'])) {
    $user_role_id = intval($_COOKIE['user_role']); // Convert to integer for safety
    
    // Get database connection
    $con = dbCon();
    
    // Escape the user role ID to prevent SQL injection
    $user_role_id = mysqli_real_escape_string($con, $user_role_id);
    
    // Execute the query
    $query = "SELECT page_permissions FROM zw_user_roles WHERE id = '$user_role_id'";
    $getPermission = mysqli_query($con, $query);
    
    // Check if query was successful
    if ($getPermission === false) {
        error_log("Failed to get permissions: " . mysqli_error($con));
        $permissions = ''; // Set default empty permissions
    } else {
        // Check if any rows were returned
        if (mysqli_num_rows($getPermission) > 0) {
            $result = mysqli_fetch_assoc($getPermission);
            $permissions = isset($result['page_permissions']) ? $result['page_permissions'] : '';
        } else {
            error_log("No permissions found for user role: " . $user_role_id);
            $permissions = ''; // No permissions found
        }
    }
} else {
    // No user role cookie found or invalid
    error_log("No valid user_role cookie found");
...
<script>
   $(document).ready(function() {
    ...
    $('.submenu').click(function() {
        ...
        $('.submenubox').addClass('d-none');
        if (this.classList.contains('issact')) {
...
<style>
    @media(max-width:720px){.mfb{font-size: large!important;padding:5px 0px;} .mm-0{margin:0!important;}  .mmy-4{margin-top:21px!important;margin-bottom:21px!important;}}
</style>


<script src="https://rawgit.com/eKoopmans/html2pdf/master/dist/html2pdf.bundle.js"></script>

<button class='rtbtn d-lg-block d-none' style='position:fixed;bottom: 2%;left: 1px;width:min-content;z-index:999;background:none;color: #004d2a;border: 0;padding: 2px 21px;font-size: 151%;transform: scale(0.69);'> <i id='voivon' class='mi-chevron-double-left'></i> </button>

<button class='rtbtnx d-lg-none d-block' style='position:fixed;top: 2%;right: 1px;width:min-content;z-index:999;background: #fff;color: #000;border: 1px solid gray;padding: 11px 21px;font-size: 182%;transform: scale(0.69);padding-bottom: 14px;border-radius: 13px;'> = </button>

<div class='col-md-2 d-lg-block d-none' id='leftmenu' style='border-right: 1px solid #cecece;padding:3% 0%;display:block;overflow-y:scroll;overflow-x:hidden;z-index:99;background:#fff;color:#000;position:fixed;left:0;top:0;bottom:0;'>
    <menulist style='overflow-x:hidden;'>
        <center><img src='https://tidyrabbit.com/assets/logo-D9R6baMe.svg' class='headlogo' style=''></center>
        <br>
        <style>
            menu {
        ...
        <menu>
            ...
            <div class='d-none submenubox' id='managepurch'>
                <div style='display:block;width:100%;margin-left:15%;font-size:80%;'>
                    ...
                    <a href='manage.php?t=expense&g=prc' class='expenseXbt'>Expenses</a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Safely handle permissions - ensure it's a valid string
        var permissionsData = <?php echo json_encode($permissions); ?>;
        accessArray = permissionsData || ''; // Default to empty string if null
        ...
        if (matchValueEdit && !(accessArray.includes(matchValueEdit + '_edit'))) {
            editPermission = false;
        }

        let matchValueDelete = storeUrlString(currentPageUrl)
        if (matchValueDelete && !(accessArray.includes(matchValueDelete + '_delete'))) {
