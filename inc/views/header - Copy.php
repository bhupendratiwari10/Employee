...
$user_role_id = $_COOKIE['user_role']; // User Role
$getPermission = generalQuery("SELECT page_permissions from zw_user_roles where id = $user_role_id");
$result = mysqli_fetch_assoc($getPermission);
$permissions = $result['page_permissions'];
...
<style>
    ...
    .mini .submenubox{display:none!important;}
...
<div class='col-md-2' id='leftmenu' style='border-right: 1px solid #cecece;padding:3% 0%;display:block;overflow-y:scroll;overflow-x:hidden;z-index:99;backgroundi:#111;color:#000;position:fixed;left:0;top:0;bottom:0;'>
    <menulist style='overflow-x:hidden;'>
        <center><img src='http://zwglobal.in/assets/zwnewlogo.png' class='headlogo' style=''></center>
        <br>
        <style>
            menu {
...
-->
            ...
            <a class='mainmenu d-none repX' href='reports.php'><i class='mi-local_police'></i>
                <l>Reports</l>
            </a>
...
$(document).ready(function() {
    ...
    $anchorTags.each(function() {
        ...
        if (href !== undefined && href.includes('?')) {
            ...
            else if (!(accessArray.includes(matchValue + '_edit'))){
            console.log((matchValue + '_edit'));
            console.log(accessArray);
            console.log(editPermission , !(accessArray.includes(matchValue + '_edit') , (matchValue + '_edit')));
            // var hrefString = "a[href=edit.php?type="+matchValue+"]";
            // var hrefString = "a[href='edit.php?type=" + matchValue + "']";
            // editAnchorTag = $(hrefString);
            // console.log(editAnchorTag , editAnchorTag.length);
            // if(editAnchorTag.length <= 0){
            // console.log(editAnchorTag.length);
