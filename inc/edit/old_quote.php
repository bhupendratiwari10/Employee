...
$sqlItems = "select  * FROM zw_Quote_items WHERE quote_id = $res[id]";
$dataItems = mysqli_query($con , $sqlItems);
$resItmes = mysqli_fetch_assoc($dataItems);
echo "<pre>"; print_r($dataItems); echo "</pre>";die;
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $subTotal = $_POST['subTotal'];
    $sqlInvoice = "Update zw_Quote set customer_id = '$customer_id', subject = '$subject', subTotal = '$subTotal' , quote_date = '$invoice_date', due_date = '$due_date', terms ='$terms', sales_person ='$sales_person', remarks ='$remarks', toc ='$toc', mail_to ='$mail_to' where id= $uid";
   ...
   mysqli_query($con, $sqlInvoice);
    ...
    for ($i = 0; $i < count($itemIds); $i++) {
        ...
        $sqlItems = "Update zw_Quote_items set quote_id ='$invoice_id' , item_id ='$itemId', quantity ='$quantity', rate ='$rate', discount ='$discount' where id = $uid";
...
?>
    ...
    <form method="post" class="row" action="">
        ...
        <div id="items">
            ...
            <?php 
           if (mysqli_num_rows($dataItems) > 0) {
    while ($itemData = mysqli_fetch_assoc($dataItems)) { ?>
