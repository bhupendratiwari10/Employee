...
$query = "SELECT * FROM `zw_epr_po` where id = $uid";
$data = mysqli_query($con, $query);
$res = mysqli_fetch_assoc($data);

$sqlItems = "SELECT * FROM zw_epr_po_items WHERE invoice_id = $uid";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $deleteItemsQuery = "DELETE FROM zw_epr_po_items WHERE invoice_id = $uid";    mysqli_query($con, $deleteItemsQuery);
    $customer_id = $_POST["customer_id"];
    $po_id = $_POST["po_no"]; 
    $po_date = $_POST["po_date"];
    $terms = $_POST["terms"];
    $total_amount = $_POST['total_amount'];
    $file = uploadImage("epr_po", "uploads/epr_po"); if(!empty($file)){$suba = "file_src = '$file', ";}else{$suba='';}
    
    $sqlInvoice = "UPDATE zw_epr_po
    ...
    for ($i = 0; $i < count($itemIds); $i++) {
        ...
        $sqlItems = "INSERT INTO zw_epr_po_items (invoice_id, item_id, description, quantity, rate,gst, states, category)
                VALUES ('$uid', '$itemId','$item_desc', '$quantity', '$rate', '$gst', '$state', '$category')";
...
<h2><i class="mi-mi-list"></i> Edit EPR P.O.</h2>
<hr/>
            ...
            <div class="row">
            	     ...
            	     <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            ...
                            <select id="customer_id" class='form-select' name="customer_id" required>
                                ...
                                <?php optionPrintAdv("zw_customers WHERE customer_type='epr' AND status='1'", "id", "customer_display_name", $res['customer_id']); ?>
                            </select>
                        </div>
                    </div>
            	</div>
                <div class="col-sm-12">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <input type="text" id="po_no" name ="po_no" class="form-control" placeholder="000-111" value='<?php echo $res['po_id']; ?>' required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <input type='date' class='form-control' name='po_date' value='<?php echo $res['po_date']; ?>' required>
                        </div>
                        <label for="" class="col-sm-2 col-form-label">Payment Terms</label>
                        <div class="col-sm-3">
                            <select class="form-control" name="terms">
                                <option value="<?php echo $res['terms']; ?>" seleted><?php echo $res['terms']; ?></option>
                    ...
                    <div class="col-sm-12 mt-5 p-0">
                        ...
                        <table class="table table-bordered table_row" width="100%">
                            ...
                            <tr>
                                ...
                                <textarea name="item_desc[]" placeholder="Description"><?php echo $resItmes['description']; ?></textarea>
                                </td>
                                <td width= '15%'><select class="form-control" name="category[]" required >
                                <option selected disabled value="">Select category</option>
                                <?php optionPrintAdv("zw_pickup_categories", "title", "title", $resItmes['category']); ?>
		                   ...
		                   <select class="form-control" name="states[]" required >
		                   	<option selected disabled value="">Select state</option>
                                ...
                                <td width='10%'><input type="text" class="col-md-12 quantity" name="quantity[]" value="<?php echo $resItmes['quantity']; ?>" placeholder="Quantity" required>
                                </td>
                                
                                <td width= '10%'><input type="text" class="col-md-12 rate" name="rate[]" value="<?php echo $resItmes['rate']; ?>" placeholder="Rate" required></td>
                                <td width= '10%'><input type="text" class="col-md-12 gst" name="gst[]" value="<?php echo $resItmes['gst']; ?>" placeholder="GST" required>
                                </td>
