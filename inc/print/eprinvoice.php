...
$id = $_GET['id']; $query = "SELECT * FROM zw_epr_invoices WHERE id='$id' LIMIT 1";  $result = mysqli_query($con, $query);
...
echo"  
    <div class='col-12 row' style='margin-top:5%; padding:2vh 5vw;padding-bottom:8vh'>
        ...
        $queriy = "SELECT * FROM zw_epr_invoice_items WHERE invoice_id='$id'";  
        $resulit = mysqli_query($con, $queriy); $nn = 0; $tt = 0;$tlt2=0;$ds=0;
        while($vk=mysqli_fetch_assoc($resulit)){ 
        $nn = $nn+1; 
        $itid = $vk['item_id']; 
        $qq = $vk['quantity']; 
        ...
        $gst = $vk['gst']; 
        $rr = $vk['rate']; 
        //$prc=namebyAid($itid,"selling_price","zw_items"); 
        $tlt = ($qq*$rr) + ($gst * $qq * $rr)/100 ;
        //$tlt = $qq*$rr; 
        $ds = $ds+0;
        $tlt2 = $tlt2+$tlt;
            echo"<div class='col-md-12 row' style='margin:5px 0px;'>"; 
                echo"<div class='col-md-1'>$nn</div>";
                echo"<div class='col-md-3'>".$itid."</div>";
                echo"<div class='col-md-2'>".namebyAid($category,"title","zw_pickup_categories")."</div>";
                echo"<div class='col-md-2'>".$states." </div>";
